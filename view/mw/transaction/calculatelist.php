...
<html>
	...
	<script type="text/javascript">
			...
			function callExcel() {
				...
				f.action = "<?=$PROC_BASE?>/transaction/calculatelistSearch_xls.php";
				f.method = "post";
				f.target = "_self";
				f.submit();
			...
			function callList(page){
				...
				var params = {page:page, perPage:perPage, fromDate:$("#fromDate").val(), toDate:$("#toDate").val(), calno:$("#cal_no").val() };
				

				$schList.empty().append('<tr ><td colspan="10" align = "center">조회 중</td></tr>');
				$totalCnt.text('0');

				Ajax.request('<?=$PROC_BASE?>/transaction/calculatelistSearch.php', params, function(res) {
					if (!Ajax.checkResult(res)) {
						$schList.empty().append('<tr class="t-c"><td colspan="10" align="center">'+ res.err_msg +'</td></tr>');
						return;
					}
		    ...
		    <div class="sub-contaniner">
			 <section class="sub-cont">
				 ...
				 <div class="cont-box sub-title-wrap">
				     <h3>정산내역조회<span>기간별 정산등록 내역을 조회 할 수 있습니다. </span></h3>
				 </div>
