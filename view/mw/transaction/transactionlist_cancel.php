...
<html>
	...
	<script type="text/javascript">
			...
			function callExcel() {
				...
				f.action = "<?=$PROC_BASE?>/transaction/transactionlist_xls_cancel.php";
				f.method = "post";
				f.target = "_self";
				f.submit();
			...
			function callList(page){
				...
				$schList.empty().append('<tr ><td colspan="13" align = "center">조회 중</td></tr>');
				$totalCnt.text('0');

				Ajax.request('<?=$PROC_BASE?>/transaction/transactionlistSearch_cancel.php', params, function(res) {
					if (!Ajax.checkResult(res)) {
						$schList.empty().append('<tr class="t-c"><td colspan="13" align="center">'+ res.err_msg +'</td></tr>');
						return;
					}
					...
					$.each(res.datalist, function(key, value){
						...
						html += '<td class="t-c"><span>'+ echoNull2Blank(value.canceldate) +'</span></td>';
						html += '<td class="t-c"><span>'+ echoNull2Blank(value.canceltime) +'</span></td>';
		    ...
		    <div class="sub-contaniner">
			 <section class="sub-cont">
				 ...
				 <div class="cont-box sub-title-wrap">
				     <h3>취소거래조회<span>기간별, 통화별 각 취소내역을 상세하게 조회하실 수 있습니다.</span></h3>
				 </div>
